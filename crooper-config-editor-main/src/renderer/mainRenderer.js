...
function renderAliasQuickAccess() {
    ...
    if (aliases.length === 0) {
        ...
        aliases.sort().forEach(aliasKey => {
            ...
            // Nouveau : structure visuelle améliorée
            aliasBlock.innerHTML = `
                <span class="alias-label">${aliasKey}</span>
                <button class="edit-alias-btn" aria-label="Éditer ${aliasKey}">⚙️</button>
            `;
            aliasBlock.querySelector('.edit-alias-btn').addEventListener('click', (e) => {
                e.stopPropagation();
                openEditProductModal(aliasKey);
            });
...
// --- UI/UX REWORK: THEME, TOASTS, SIDEBAR SEARCH ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (themeBtn) {
        themeBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('crooper-theme', document.body.classList.contains('dark') ? 'dark' : 'light');
        });
        // Load theme from storage
        if (localStorage.getItem('crooper-theme') === 'dark') {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            ...
            // Filtrage alias
            const aliasBlocks = document.querySelectorAll('.alias-block');
            aliasBlocks.forEach(block => {
                const alias = block.textContent?.toLowerCase() || '';
                block.style.display = alias.includes(query) ? '' : 'none';
            });
        });
    }
});
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Cependant, garder le console.error pour la visibilité.
    for (const key in elements) {
    ...
    // Initialise modules with leur DOM éléments respectifs
...
// --- ACCESSIBILITÉ & UX MODALES ---

document.addEventListener('DOMContentLoaded', () => {
    // ACCESSIBILITÉ : focus visible sur modales
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            ...
            if (openModal) {
                const closeBtn = openModal.querySelector('.closeButton, .close, [data-close]');
    ...
    // Amélioration UX : focus automatique sur recherche sidebar
    const searchInput = document.getElementById('sidebarSearchInput');
    if (searchInput) {
